...
import { Eye, EyeOff, Mail, Lock, User, Phone, ArrowRight, Printer } from 'lucide-react'
...
function AuthInput({
  label, icon, error, rightElement, leftPrefix, ...props
}: {
  label: string
  ...
  return (
    <div>
      ...
      >
        <div className="absolute left-4 text-white/30 pointer-events-none flex items-center gap-2">
          {icon}
          {leftPrefix && <span className="text-xs text-white/30 border-r border-white/10 pr-2">{leftPrefix}</span>}
        </div>
        <input
          className="w-full h-12 bg-transparent pl-11 pr-11 text-sm text-white placeholder:text-white/25 outline-none focus:ring-0"
          style={{ paddingLeft: leftPrefix ? '5.5rem' : undefined, caretColor: '#2D6FFF' }}
          {...props}
        />
        {rightElement && (
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    addToast({ type: 'info', title: 'Supabase not configured', message: 'Add your credentials to .env.local to enable sign-up.' })
    setLoading(false)
    console.log(data)
  }
        ...
        >
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <div className="h-px w-6" style={{ background: 'linear-gradient(90deg, #F5C518, transparent)' }} />
              <span className="text-xs font-semibold text-brand-gold tracking-[0.2em] uppercase">New Account</span>
            </div>
            <h1 className="font-display font-bold text-3xl text-white leading-tight">
              Join JPP.
            </h1>
            <p className="text-sm text-white/40 mt-2">
              Track orders, get bulk pricing, and print smarter.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button
              ...
              className="relative w-full h-12 rounded-xl font-semibold text-sm text-white flex items-center justify-center gap-2 overflow-hidden transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed group mt-2"
              style={{
          ...
          <p className="text-[11px] text-white/25 text-center mt-4 leading-relaxed">
            ...
            <Link href="/terms" className="text-white/40 hover:text-brand-blue transition-colors underline underline-offset-2">Terms</Link>
            {' '}and{' '}
            <Link href="/privacy" className="text-white/40 hover:text-brand-blue transition-colors underline underline-offset-2">Privacy Policy</Link>.
          </p>
